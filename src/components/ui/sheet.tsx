...
/**
 * Side panel. Radix portals it to `document.body`, which is what keeps it out of the app
 * shell's `zoom: 0.8` wrapper — a fixed overlay rendered inside that wrapper has its pixel
 * offsets rescaled and lands in the wrong place.
 */
function SheetContent({
  ...
  /** Accessible name of the dismiss control; pass a translated string on localized panels. */
  closeLabel?: string
}) {
  return (
    <SheetPortal data-slot="sheet-portal">
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          "bg-background data-[state=open]:animate-in data-[state=closed]:animate-out fixed z-[70] flex flex-col gap-4 shadow-lg outline-none transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
          side === "right" &&
