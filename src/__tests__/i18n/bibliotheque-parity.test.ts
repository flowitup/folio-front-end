/**
 * bibliotheque-parity.test.ts
 *
 * Asserts that en.json, fr.json, and vi.json have identical key trees under:
 *   - bibliotheque.*
 *   - navigation.bibliotheque (single key added to navigation namespace)
 *
...
const NAMESPACES = ["bibliotheque", "navigation"] as const;
...
describe("i18n bibliotheque parity", () => {
  ...
  // Verify navigation.bibliotheque key is present in all locales
  describe("navigation.bibliotheque key", () => {
    for (const locale of LOCALES) {
      it(`${locale.name} has navigation.bibliotheque`, () => {
        const val = dig(locale.messages, "navigation.bibliotheque");
        expect(val, `navigation.bibliotheque missing in ${locale.name}`).toBeTruthy();
